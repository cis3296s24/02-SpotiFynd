...
def generate_user_tracks(limit=50):
    ...
    song_recommendations = spotify.recommendations(seed_tracks=seeded_tracks, seed_genres=[most_common_genre],
                                                   limit=limit)

    # Prepare the data for create_dataframe
    track_data = []
    recommended_track_ids = [track['id'] for track in song_recommendations['tracks'] if
                             track['id'] not in saved_track_ids]
